...
const WorkspaceContents = (count = 10, monitorId) => {
  return DrawingArea({
    ...
    attribute: {
      ...
      workspaceGroup: 0,
      updateMask: (self) => {
        const offset = monitorId * count; // Calculate the offset based on the monitor ID
    ...
    setup: (area) => area
      .hook(Hyprland.active.workspace, (self) => {
        ...
        if (activeWs > offset && activeWs <= offset + count) {
          self.setCss(`font-size: ${(activeWs - offset)}px;`);
        }
...
export default (monitorId) => EventBox({
  ...
  setup: (self) => {
    // Add event mask for pointer motion and button press
